...
class _FamilyDetailsScreenState extends ConsumerState<FamilyDetailsScreen> {
  ...
  void _addFamilyMember() {
    showModalBottomSheet(
      ...
      builder: (context) => _AddFamilyMemberBottomSheet(
                              ...
                              child: Row(
                                children: [
                                  ...
                                  Container(
                                    ...
                                    height: 54,
                                    decoration: BoxDecoration(
                                      color: MMPApp.maroon.withValues(alpha: 0.08),
  ...
  Widget _buildEmptyState() {
    return Center(
      child: Column(
        ...
        children: [
          Icon(Icons.family_restroom, size: 80, color: Colors.grey.withValues(alpha: 0.5)),
...
class _AddFamilyMemberBottomSheet extends StatefulWidget {
  final Map<String, dynamic>? initialData;
  final Function(Map<String, dynamic>, File?) onAdd;

  const _AddFamilyMemberBottomSheet({
    this.initialData,
    required this.onAdd,
  });

  @override
  State<_AddFamilyMemberBottomSheet> createState() => _AddFamilyMemberBottomSheetState();
}

class _AddFamilyMemberBottomSheetState extends State<_AddFamilyMemberBottomSheet> {
  ...
  String _formatDate(DateTime? d) => d == null ? 'Select Date' : DateFormat('dd MMM yyyy').format(d);

  String? _apiDate(DateTime? d) => d?.toIso8601String().split('T').first;
              ...
              Row(
                ...
                children: [
                  Text(
                    ...
                    style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: MMPApp.maroon),
                  ),
                  IconButton(
  ...
  Widget _dateTile(String label, bool isDob) {
    return ListTile(
      contentPadding: EdgeInsets.zero,
      leading: Icon(isDob ? Icons.cake : Icons.favorite, color: MMPApp.maroon),
      title: Text(label),
    ...
    showModalBottomSheet(
      ...
      builder: (_) => SafeArea(
        child: Column(
          ...
          children: [
            ListTile(
              leading: const Icon(Icons.camera_alt, color: MMPApp.maroon),
            ...
            ListTile(
              leading: const Icon(Icons.photo, color: MMPApp.maroon),
            ...
            Center(
              child: CircleAvatar(
                ...
                child: (member.image == null || member.image!.isEmpty)
                    ? const Icon(Icons.person, size: 60, color: MMPApp.maroon)
                    : null,
              ),
            ),
            const SizedBox(height: 24),
